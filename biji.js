...
todo 然后，将 .babelrc 内容修改为：（是往里加内容，不是替换掉）

{
    "presets": [ ["es2015", { "modules": false }] ], //todo 这里 es2015 改成 @babel/preset-env即可，这是UI组件库没跟上vue-cli的更新
    "plugins": [
    [
        "component",
        {
...
new Vue({
 ...
 todo 自定义主题   参见day07 第18讲


 仅替换主题色
 如果仅希望更换 Element 的主题色，推荐使用在线主题生成工具。Element 默认的主题色是鲜艳、友好的蓝色。
 通过替换主题色，能够让 Element 的视觉更加符合具体项目的定位。

 使用上述工具，可以很方便地实时预览主题色改变之后的视觉，同时它还可以基于新的主题色生成完整的样式文件包，
 供直接下载使用（关于如何使用下载的主题包，请参考本节「引入自定义主题」和「搭配插件按需引入组件主题」部分）。
 TODO 把下载的压缩包解压 重命名为theme111   放在根目录下 与src同级
 TODO "styleLibraryName": "theme-chalk"    配置项改为   "styleLibraryName": "~theme111"
 TODO 若放在src下  配置应为   "styleLibraryName": "~src/theme"
 完整配置为
 module.exports = {
  presets: [
